...
class Mapper():
    ...
    def __init__(self, robot):
        ...
        self.obs_views = np.zeros((self.MAX_CELLS_X, self.MAX_CELLS_Y,
                                   self.MAX_CELLS_A, self.OBS_PER_CELL))
        # Ray Intersection points based on the map
        self.obs_points_x = np.zeros((self.MAX_CELLS_X, self.MAX_CELLS_Y,
                                      self.MAX_CELLS_A, self.OBS_PER_CELL))
        self.obs_points_y = np.zeros((self.MAX_CELLS_X, self.MAX_CELLS_Y,
                                      self.MAX_CELLS_A, self.OBS_PER_CELL))
    ...
    def get_intersection(self, ray, pose):
        try:
            with np.errstate(divide='ignore'):
                ...
                intersections_tt = ray[0] + tt[:,
                                               np.newaxis] * (ray[1] - ray[0])
    ...
    def populate_views(self):
        ...
        for cx in range(0, self.MAX_CELLS_X):
            for cy in range(0, self.MAX_CELLS_Y):
                for ca in range(0, self.MAX_CELLS_A):
                    ...
                    tracing_rays = self.get_tracing_rays(
        ...
        LOG.info(" | Precaching Time: {:.3f} secs".format(time.time() -
                                                          start_time))
...
class BaseLocalization():
    ...
    def __init__(self, robot, mapper):
        ...
        self.bel_bar = np.zeros(
            (self.mapper.MAX_CELLS_X, self.mapper.MAX_CELLS_Y,
             self.mapper.MAX_CELLS_A))
        self.bel = np.zeros((self.mapper.MAX_CELLS_X, self.mapper.MAX_CELLS_Y,
                             self.mapper.MAX_CELLS_A))
        ...
        self.odom_trans_sigma = self.robot.config_params["localization"][
            "odom_trans_sigma"]
        self.odom_rot_sigma = self.robot.config_params["localization"][
            "odom_rot_sigma"]
        self.sensor_sigma = self.robot.config_params["localization"][
            "sensor_sigma"]
    ...
    def init_uniform_distribution(self, x, y, a):
        ...
        LOG.info("Uniform Belief with each cell value: {}".format(self.bel[0, 0,
                                                                           0]))
    ...
    def print_prediction_stats(self, plot_data=True):
        ...
        LOG.info("Prior Bel index  : {} with prob = {}".format(
            argmax_bel_bar[0],
    ...
    def print_update_stats(self, plot_data=True):
        ...
        # Print prob as a string to prevent rounding
        LOG.info("GT index      : {}".format(gt_index))
        LOG.info("Bel index     : {} with prob = {}".format(
            argmax_bel[0],
            str(argmax_bel[1])[:9]))
        ...
        LOG.info(
            "Belief        : ({:.3f}, {:.3f}, {:.3f})".format(*current_belief))
